...
function usage() {
  console.error('Usage: node scripts/audit-qonto-postman.mjs <postman.json> [splitOutDir] [reportPath]');
  process.exit(1);
}
...
function extractPostmanParams(request) {
  ...
  for (const q of query) {
    if (q && !q.disabled && typeof q.key === 'string' && q.key.trim()) params.query.add(q.key.trim());
  }
...
function parseNodeOperations(sourceCode) {
  ...
  for (const resourceMatch of sourceCode.matchAll(resourceRegex)) {
    ...
    for (const opMatch of resourceBlock.matchAll(opRegex)) {
      ...
      const endpointMatch = opBlock.match(/const endpoint\s*=\s*`([^`]+)`/) || opBlock.match(/const endpoint\s*=\s*'([^']+)'/);
      ...
      for (const m of opBlock.matchAll(/([a-zA-Z0-9_]+)\s*:\s*this\.getNodeParameter\('([^']+)'/g)) {
...
function createAudit(postmanRequests, nodeOps) {
  ...
  for (const op of nodeOps) {
    ...
    const postmanParams = toSet([
      ...req.params.path,
      ...req.params.query,
      ...req.params.body,
    ]);

    const codeApiParams = toSet([
      ...op.params.path,
      ...op.params.query,
      ...op.params.bodyApi,
    ]);
...
function writeMarkdownReport(reportJsonPath, markdownPath) {
  ...
  for (const item of report.missingInNode.slice(0, 50)) {
    lines.push(`- ${item.method} ${item.pathTemplate} (${item.postmanCollection} -> ${item.postmanRequest})`);
  }
  if (report.missingInNode.length > 50) lines.push(`- ... ${report.missingInNode.length - 50} more`);
  lines.push('');
  ...
  for (const item of paramIssues.slice(0, 50)) {
    ...
    if (item.missingQueryParamsInCode.length) {
      lines.push(
        `  - Missing query params in code: ${item.missingQueryParamsInCode.join(', ')}`,
      );
    }
