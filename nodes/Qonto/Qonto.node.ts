...
import { formatDateTime, formatDate, handleListing, qontoApiRequest } from './helpers';
			...
			try {
				...
				if (resource === 'externalTransfers') {
					...
					if (operation === 'createExternalTransfersWithCreditorData') {
						...
						const idempotencyKey = this.getNodeParameter('idempotency_key', i, null) as
							| string
							| null;
						headers = {
							...headers,
					...
					if (operation === 'listExternalTransfers') {
						...
						if (!isEmpty(filters)) {
							...
							if (filters.recurring_transfer_ids) {
								const rIds = (filters.recurring_transfer_ids as string)
									.split(',')
									.map((id) => id.trim());
				...
				if (resource === 'sepaTransfers') {
					if (operation === 'listSepaTransfers') {
						...
						const queryParams = parseJsonParameter(
							this.getNodeParameter('query', i, {}) as unknown,
						);
					...
					if (operation === 'createSepaBulkTransfer') {
						...
						const body = parseJsonParameter(
							this.getNodeParameter('bulkTransferPayload', i) as unknown,
						);
						responseData = await qontoApiRequest.call(this, headers, 'POST', endpoint, body, {});
					}
					...
					if (operation === 'createSepaRecurringTransfer') {
						const endpoint = 'sepa/recurring_transfers';
						const body = parseJsonParameter(
							this.getNodeParameter('recurringTransferPayload', i) as unknown,
						);
						responseData = await qontoApiRequest.call(this, headers, 'POST', endpoint, body, {});
					}
					...
					if (operation === 'verifySepaPayee') {
						...
						const body = parseJsonParameter(
							this.getNodeParameter('verifyPayeePayload', i) as unknown,
						);
						responseData = await qontoApiRequest.call(this, headers, 'POST', endpoint, body, {});
					}
					...
					if (operation === 'bulkVerifySepaPayee') {
						...
						const body = parseJsonParameter(
							this.getNodeParameter('bulkVerifyPayeePayload', i) as unknown,
						);
						responseData = await qontoApiRequest.call(this, headers, 'POST', endpoint, body, {});
					}
					...
					if (operation === 'skipVerifySepaPayee') {
						...
						const body = parseJsonParameter(
							this.getNodeParameter('skipVerifyPayeePayload', i) as unknown,
						);
						responseData = await qontoApiRequest.call(this, headers, 'POST', endpoint, body, {});
					}
				}
				...
				if (resource === 'organizations') {
					...
					if (operation === 'getOrganizationAndItsBank_accounts') {
						...
						const includeExternalAccounts = this.getNodeParameter(
						...
						// Appel de l'API
						responseData = await qontoApiRequest.call(this, headers, 'GET', endpoint, {}, query);
					}
				}
				...
				if (resource === 'transactions') {
					...
					if (operation === 'showTransaction') {
						...
						responseData = await qontoApiRequest.call(this, {}, 'GET', endpoint, {}, showQuery);
					}
				}
				...
				if (resource === 'requests') {
					...
					if (operation === 'createFlashCardRequest') {
						...
						const body: IDataObject = {
							request_flash_card: {
								note: this.getNodeParameter('note', i, '') as string,
								payment_lifespan_limit: this.getNodeParameter(
									'payment_lifespan_limit',
									i,
								) as number,
								pre_expires_at: formatDateTime(
									this.getNodeParameter('pre_expires_at', i) as string,
								),
							},
						};
				...
				if (resource === 'clientsInvoices') {
					...
					if (operation === 'listInvoices') {
						...
						if (!isEmpty(filters)) {
							...
							if (filters.created_at_from) {
								query['filter[created_at_from]'] = formatDateTime(
									filters.created_at_from as string,
								);
							}
							if (filters.created_at_to) {
							...
							if (filters.updated_at_from) {
								query['filter[updated_at_from]'] = formatDateTime(
									filters.updated_at_from as string,
								);
							}
							if (filters.updated_at_to) {
					...
					if (operation === 'createClientInvoice') {
						...
						if (additionalOptions.performanceStartDate) {
							body.performance_start_date = formatDate(
								additionalOptions.performanceStartDate as string,
							);
						}
						if (additionalOptions.performanceEndDate) {
							body.performance_end_date = formatDate(
								additionalOptions.performanceEndDate as string,
							);
						}
				...
				if (resource === 'clients') {
					...
					if (operation === 'createClient') {
						...
						if (additionalOptions.extraEmails) {
							body.extra_emails = (additionalOptions.extraEmails as string)
								.split(',')
								.map((e: string) => e.trim());
						}
						if (
							...
							if (additionalOptions.billingStreetAddress) {
								(body.billing_address as IDataObject).street_address =
									additionalOptions.billingStreetAddress;
							}
							...
							if (additionalOptions.billingProvinceCode) {
								(body.billing_address as IDataObject).province_code =
									additionalOptions.billingProvinceCode;
							}
							if (additionalOptions.billingCountryCode) {
								(body.billing_address as IDataObject).country_code =
									additionalOptions.billingCountryCode;
							}
						}
				...
				if (resource === 'cards') {
					...
					if (operation === 'lockCard') {
						...
						responseData = await qontoApiRequest.call(this, headers, 'PUT', endpoint, {}, {});
					}
